		...
		public function output() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				foreach ( $options as $key => $value ) {
					echo '<label><input type="' . esc_attr($input_type) . '" name="' . esc_attr($this->element_name( $input_attr )) . '" value="' . esc_attr($key) . '"' . esc_attr($this->element_class()) . wp_kses($this->element_attributes( $key ) . $this->checked( $this->element_value(), $key ), $this->allowedTags) . '/>';
					echo ( ! empty( $this->field['text_select'] ) ) ? '<span class="exopite-sof-' . esc_attr(sanitize_title( $value )) . '">' . esc_html($value) . '</span>' : '<img src="' . esc_url($value) . '"   alt="' . esc_attr($key) . '" />';
					echo '</label>';
				}
			}
